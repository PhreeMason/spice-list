...
const Settings = ({ className }: { className?: string }) => (
  <MaterialCommunityIcons name="cog-outline" size={24} className={className} />
);
...
const TabItem = ({title, subtitle}: {title: string; subtitle: string | number}) => (
  <TouchableOpacity className="flex-row text-gray-500 gap-2">
    <Text className='font-spice-semibold'>{title}</Text>
    ...
    <View className="bg-white min-h-screen font-sans">
      <View className="p-4">
        ...
        <View className="flex-row justify-between mb-4">
          ...
          <TabItem title="Books" subtitle={profileData.books} />
          <TabItem title="Lists" subtitle={profileData.lists} />
          <TabItem title="Quotes" subtitle={profileData.quotes} />
        </View>
      </View>
    </View>
  );
