...
const generateUrl = (slug: string) => `https://www.goodreads.com/search?q=${encodeURIComponent(
    slug
)}`;

function extractBookInfo($: cheerio.CheerioAPI) {
    const cover = $('.BookCover__image').attr('src');
    const title = $('.Text__title1').text().trim();
    const bookUrl = $('link[rel="canonical"]').attr('href');
    const authors = $('.ContributorLinksList .ContributorLink').map((_, el) => {
        return {
            name: $(el).text().trim(),
            url: $(el).attr('href')
        };
    ...
    const rating = $('.RatingStatistics__rating:first').text().trim();
...
function extractBookInfoFromBookPage($: cheerio.CheerioAPI, res: express.Response) {
